...
document.addEventListener('DOMContentLoaded', function() {
    ...
    carrito.addEventListener('click', async function(event) {
        ...
        const userDocRef = doc(firestore, 'Productos', 'Accesorios1');
        const docSnap = await getDoc(userDocRef);
        ...
        if (docSnap.exists() && docSnap1.exists()) {
            ...
            const datos = {
                ...
                Precio: precio1,
                Nombre:nombre1,
                ID_DOCUMENTO: "Accesorios1",
                Cuantos: suma,
                Producto:producto
            };
          
            const peticionesRef = collection(nuevoDocRef, 'peticiones');
            const Celular1DocumentoRef = doc(peticionesRef, 'Accesorios1'); // Aquí defines el nombre del documento
