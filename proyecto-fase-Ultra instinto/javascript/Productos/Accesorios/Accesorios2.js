...
actualizarDocumento(2, "Accesorios1", "Nombre1", "Precio1", "Descuento1");

document.addEventListener('DOMContentLoaded', function() {
    const carrito = document.getElementById('Carrito1');
    carrito.addEventListener('click', async function(event) {
        event.preventDefault();
        const userDocRef = doc(firestore, 'Productos', 'Accesorios2');
        const docSnap = await getDoc(userDocRef);
        ...
        if (docSnap.exists() && docSnap1.exists()) {
            ...
            const datos = {
                ...
                Precio: precio1,
                Nombre:nombre1,
                ID_DOCUMENTO: "Accesorios2",
                Cuantos: suma,
                Producto:producto
            };
          
            const peticionesRef = collection(nuevoDocRef, 'peticiones');
            const Celular1DocumentoRef = doc(peticionesRef, 'Accesorios2'); // Aquí defines el nombre del documento
