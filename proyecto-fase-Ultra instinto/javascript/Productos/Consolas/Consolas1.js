...
document.addEventListener('DOMContentLoaded', function() {
    ...
    carrito.addEventListener('click', async function(event) {
        ...
        const userDocRef = doc(firestore, 'Productos', 'Consolas1');
        const docSnap = await getDoc(userDocRef);
        ...
        if (docSnap.exists() && docSnap1.exists()) {
            ...
            const datos = {
                ...
                Precio: precio1,
                Nombre:nombre1,
                ID_DOCUMENTO: "Consolas1",
                Cuantos: suma,
                Producto:producto
            };
          
            const peticionesRef = collection(nuevoDocRef, 'peticiones');
            const Celular1DocumentoRef = doc(peticionesRef, 'Consolas1'); // Aquí defines el nombre del documento
